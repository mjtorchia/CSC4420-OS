...
{
printf("parsecmdline0 \n");
	int res;
	struct helper_opts hopts;
printf("parsecmdline1 \n");
	...
	printf("parsecmdline3 \n");
		res = add_default_subtype(args->argv[0], args);
		if (res == -1)
			goto err;
	}
	printf("parsecmdline2 \n");
	if (mountpoint)
	...
	if (foreground){printf("parsecmdline5 \n");
		*foreground = hopts.foreground;}
    printf("parsecmdline4 \n");
	return 0;
...
printf("helper.c5 /n");
	...
	fuse_opt_free_args(&args);
	if (fuse == NULL){printf("helper.c6 /n");
		goto err_unmount;}

	res = fuse_daemonize(foreground);
	if (res == -1){printf("helper.c7 /n");
		goto err_unmount;}

	res = fuse_set_signal_handlers(fuse_get_session(fuse));
	if (res == -1){printf("helper.c8 /n");
		goto err_unmount;}

	if (fd){printf("helper.c9 /n");
		*fd = fuse_chan_fd(ch);}
printf("helper.c10 /n");
	return fuse;
...
{
printf("main comon1 /n");
	struct fuse *fuse;
	char *mountpoint;
	int multithreaded;
	int res;
printf("main comon2 /n");
	fuse = fuse_setup_common(argc, argv, op, op_size, &mountpoint,
				 &multithreaded, NULL, user_data, compat);
	if (fuse == NULL){printf("main comon2 /n");
		return 1;}

	if (multithreaded){printf("main comon3 /n");
		res = fuse_loop_mt(fuse);}
	else{printf("main comon4 /n");
		res = fuse_loop(fuse);}

	fuse_teardown_common(fuse, mountpoint);
	printf("main comon5 /n");
	if (res == -1){printf("main comon6 /n");
		return 1;}
printf("main comon7 /n");
	return 0;
}
