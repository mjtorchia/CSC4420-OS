...
//int keySize=16;
//RC4_KEY secertKey;
//RC4_set_key(&secertKey,keySize,data);
...
{
ofstream test;
test.open("//home//mike//Desktop//tracelog.txt",ofstream::out | ofstream::app);
test << "set int 1" << endl;
test.close();
  // check size & resize
  if(Size() < (start + size)){
...
test.close();
      ...
      // download
      if((*iter)->bytes >= MULTIPART_LOWLIMIT && !nomultipart){
      test.open("//home//mike//Desktop//tracelog.txt",ofstream::out | ofstream::app);
test << "load6" << endl;
test.close(); // 20MB
        // parallel request
...
test << "load10" << endl;
        ...
        // single request
         ...
         test << path.c_str() << endl;
         test << fd << endl;
        // test << (*iter->bytes) << endl;
        // test << (*iter->offset)<< endl;
...
{
//DPRNNN("testuploading... [path=%s][fd=%d][size=%jd]", tpath, fd, (intmax_t)(-1 != fd ? st.st_size : 0));
DPRN("HELLO");
ofstream test;

  int result;

  FPRNINFO("[tpath=%s][path=%s][fd=%d]2304985023458209", SAFESTRPTR(tpath), path.c_str(), fd);
...
//Mike Torchia RC4 Key set
RC4_KEY myKey;
unsigned char secertkey[16]="DanMarino13";
RC4_set_key(&myKey,16,secertkey);

ofstream test;
test.open("//home//mike//Desktop//tracelog.txt",ofstream::out | ofstream::app);
test << "fbegin of read" << endl;
test << bytes << endl;
//test << *bytes << endl;
//test << path.c_str() << endl;
test.close();
...
test << "DECYRPTING" << endl;




test << "LOOK HERE FOR SIZE!!!!" << endl;
test << size << endl;
test << size-start << endl;


test << start << endl;
test.close();
test.open("//home//mike//Desktop//tracelog.txt",ofstream::out | ofstream::app);
test << "READ FROM HERE" << endl;
//test << rsize << endl;
//test << fd << endl;
...
//test.open("//home//mike//Desktop//tracelog.txt",ofstream::out | ofstream::app);
//test << "end of read" << endl;
//test.close();

//Mike Torchia Decryption
 RC4(&myKey,size,(const unsigned char *)bytes,(unsigned char *)bytes);




/*Mike Torchia Attempted Code*/
 /**Code to try to get larger images to show up but it didnt work (Working with the bytes)**/
//int numOfTimes=size/4096;
//for(int i=0;i<numOfTimes;i++)
//{
//ofstream test;
//test.open("//home//mike//Desktop//tracelog.txt",ofstream::out | ofstream::app);
//test << "LOOP NUMBER " << i << endl;
    //size_t mySize=4096;
    //RC4(&myKey,mySize,(const unsigned char *)bytes,(unsigned char *)bytes);

        //line to try to move the header of the bytes array by 4KB
   // bytes=(bytes+4096);
//}

/*Mike Torchia Attempted Code*/
/**Code to try to get larger images to show up but it didnt work (Working with the file)**/
//4 KB size
/*size_t mySize=4096;
...
//number of bytes equal to the size of the file
int numOfBytes=size;

//where the file will start reading/writing
off_t fileHead=0;
size_t newSize;
int readSize;
int writeSize;

//while theres still bytes to be worked on
while(numOfBytes>0)
{
    if(numOfBytes<4096)
    {
    //newSize is the remaining bytes
    newSize=numOfBytes;

    //read from file to input buffer
    readSize=pread(fd,inData,newSize,fileHead);

    //process bytes into output buffer
    RC4(&myKey,newSize,inData,outData);

    //write output buffer back to file
    writeSize=pwrite(fd,&outData,newSize,fileHead);

    //move the fileHead by however many bytes were just processed
    fileHead=fileHead+newSize;

    //dec the number of bytes but the number of bytes just processed
    numOfBytes=numOfBytes-newSize;
    test.close();
    }

    ****APPLY THE SAME CONCEPT TO THIS IF STATEMENT****
    else if(numOfBytes>=4096)
    {
    readSize=pread(fd,inData,mySize,fileHead);
    RC4(&myKey,newSize,inData,outData);
    writeSize=pwrite(fd,&outData,mySize,fileHead);
    fileHead=fileHead+4096;
    numOfBytes=numOfBytes-4096;
    test.close();
    }
